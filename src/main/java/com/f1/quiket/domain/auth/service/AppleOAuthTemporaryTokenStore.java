...
/**
 * Apple OAuth 임시 토큰 저장소
 */
@Component
@RequiredArgsConstructor
public class AppleOAuthTemporaryTokenStore {

    private static final String SIGNUP_TOKEN_PREFIX = "auth:apple:signup:";
    private static final String LINK_TOKEN_PREFIX = "auth:apple:link:";

    private final StringRedisTemplate stringRedisTemplate;
    private final ObjectMapper objectMapper = new ObjectMapper();

    public String saveSignup(AppleOAuthSignupTokenPayload payload, long ttlSeconds) {
        return save(SIGNUP_TOKEN_PREFIX, payload, ttlSeconds);
    }

    public Optional<AppleOAuthSignupTokenPayload> findSignup(String signupToken) {
        return find(SIGNUP_TOKEN_PREFIX, signupToken, AppleOAuthSignupTokenPayload.class);
    }
    ...
    public String saveLink(AppleOAuthLinkTokenPayload payload, long ttlSeconds) {
        return save(LINK_TOKEN_PREFIX, payload, ttlSeconds);
    }

    public Optional<AppleOAuthLinkTokenPayload> findLink(String linkToken) {
        return find(LINK_TOKEN_PREFIX, linkToken, AppleOAuthLinkTokenPayload.class);
    }
